...
corpus: list[dict[str, str]] = [
    ...
    # Nurses
    {"category": "nurse", "text": "The nurse reports to the doctor."},
    {"category": "nurse", "text": "The nurse provies care to the patient."},
    {"category": "nurse", "text": "The nurse assists the doctor."},
    # Hospitals
...
# Show results
for target in ["hospital", "doctor", "nurse", "patient"]:
    print(f"\nContext for '{target}':")
...
# to get the top 5 tokens for the "car" category.
patient_df = category_freq_df.filter(pl.col("category") == "patient").head(5)
...
plt.bar(patient_df["token"], patient_df["len"])
...
plt.title("Case Example: Top Tokens (Patient Category)")
plt.xlabel("Token")
plt.ylabel("Frequency")

# Define a new DataFrame that filters the category frequency DataFrame
# to get the top 5 tokens for the "car" category.
nurse_df = category_freq_df.filter(pl.col("category") == "nurse").head(5)

# Create a figure that is 8 inches wide and 4 inches tall
plt.figure(figsize=(8, 4))

# Add a bar chart to the figure using the tokens as the x-axis and their frequencies as the y-axis.
plt.bar(nurse_df["token"], nurse_df["len"])
...
# Set the title and labels for the axes of the plot.
plt.title("Case Example: Top Tokens (Nurse Category)")
plt.xlabel("Token")
plt.ylabel("Frequency")
